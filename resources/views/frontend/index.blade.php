...
@push('css')
    ...
    <style>
        ...
        .rekap-section {
            ...
            gap: 10px;
            margin-top: -30px;
            /* Membuat rekap sedikit naik menimpa area biru */
            z-index: 2;
            position: relative;
        }
...
@section('content')
    <div class="section" id="user-section">
        ...
        {{-- <a class="logout" id="logout-button" href="#" onclick="event.preventDefault(); document.getElementById('logout-form').submit();">
            <ion-icon name="log-out-outline"></ion-icon>
        </a> --}}
        <a class="logout" id="logout-button" href="#">
            <ion-icon name="log-out-outline"></ion-icon>
        </a>
        ...
        <div class="d-flex align-items-center">
            ...
            <div class="ms-3 d-flex flex-column justify-content-center">
                ...
                <div class="mt-1">
                    <span
                        class="badge bg-white bg-opacity-20 text-white fw-light shadow-none d-inline-flex align-items-center"
                ...
                <div class="card bg-success text-white presence-card py-3 shadow-sm">
                    <div class="card-body p-0 text-center">
                        <div class="d-flex flex-column align-items-center justify-content-center">
                            <div class="presence-img-container d-flex align-items-center justify-content-center mb-1"
                                style="width: 50px; height: 50px;">
                                @if ($presensi_today && $presensi_today->photo_in)
                                    <img src="{{ asset('storage/absensi/' . $presensi_today->photo_in) }}"
                                        class="presence-img m-0">
                                @else
                        ...
                        <div class="d-flex flex-column align-items-center justify-content-center">
                            <div class="presence-img-container d-flex align-items-center justify-content-center mb-1"
                                ...
                                @if ($presensi_today && $presensi_today->photo_out)
                                    <img src="{{ asset('storage/absensi/' . $presensi_today->photo_out) }}"
                                        class="presence-img m-0">
                                @else
                                    <ion-icon name="camera-outline" style="font-size: 32px;"></ion-icon>
                                @endif
                        ...
                        <div class="splide__slide">
                            <div class="card announcement-box border-0 shadow-sm mx-1">
                                <div class="card-body p-2">
                                    <div class="d-flex">
                                        <ion-icon name="megaphone" class="text-warning me-2"
                                            style="font-size: 20px;"></ion-icon>
                                        <div class="w-100">
                                            <div class="d-flex justify-content-between">
                                                <small class="fw-bold">{{ $info->title }}</small>
                                                <small class="text-muted"
                                                    style="font-size: 9px;">{{ date('d M', strtotime($info->created_at)) }}</small>
                                            </div>

                                            <p class="m-0"
                                                style="font-size: 11px; color: #555; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden;">
                                                {{ $info->content }}
                                            </p>

                                            <div class="d-flex justify-content-between align-items-center mt-1">
                                                <a href="javascript:void(0);" class="text-primary fw-bold btn-read-more"
                                                    style="font-size: 10px;" data-toggle="modal"
                                                    data-target="#modalAnnouncement" data-title="{{ $info->title }}"
                                                    data-content="{{ $info->content }}">
                                                    Read More...
                                                </a>
                                                @if ($info->file_name)
                                                    <a href="{{ route('download.announcement', $info->file_name) }}"
    ...
    @forelse($history_on_mount as $item)
        <div class="card mb-1 border-0 shadow-sm rounded-lg p-2">
            <div class="d-flex align-items-center">
                {{-- Ikon berubah jika status P --}}
                @if($item->status == 'P')
                    <ion-icon name="document-text" class="text-info h2 me-2"></ion-icon>
                @else
                ...
                <div class="w-100">
                    <div class="d-flex justify-content-between">
                        ...
                        {{-- Label status jika Pengajuan --}}
                        @if($item->status == 'P')
                            <span class="badge bg-info" style="font-size: 8px; letter-spacing: 0.5px;">PENGAJUAN</span>
                        @else
                            <small class="text-primary fw-bold" style="font-size: 10px;">{{ $item->name ?? 'Non-Shift' }}</small>
                        @endif
                    </div>

                    <div class="d-flex justify-content-between align-items-center">
                        <span style="font-size: 12px;" class="text-muted fw-bold">
                        ...
                        {{-- Logika Badge Status --}}
                        @if($item->status == 'P')
                            {{-- Jika status P, tidak perlu hitung terlambat --}}
                            <span class="badge bg-primary" style="font-size: 9px;">Processed</span>
                        @elseif ($batas_masuk)
                    ...
                    {{-- Tampilkan alasan jika status P (Opsional jika Anda melakukan join submissions) --}}
                    {{-- @if($item->status == 'P' && isset($item->alasan_koreksi))
                        <div style="font-size: 9px; color: #17a2b8; font-style: italic;" class="mt-1">
                            "{{ $item->alasan_koreksi }}"
                        </div>
                    @endif --}}
                </div>
            ...
            <div class="tab-pane fade" id="leader">
                <ul class="listview image-listview shadow-sm rounded-xl overflow-hidden">
                    @foreach ($leader_board as $l)
                        <li>
                            <div class="item">
                                <img src="{{ $l->avatar ? asset('storage/' . $l->avatar) : asset('assets/img/avatar.png') }}"
                                    class="image">
                                <div class="in">
                                    <div>
    ...
    <script>
        ...
        $('#logout-button').click(function(e) {
            ...
            Swal.fire({
                ...
                confirmButtonColor: '#4e73df', // Warna biru primer
