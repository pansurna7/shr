...
@forelse ($presences as $d)
    <tr>
        ...
        <td class="align-middle">
            <span class="badge bg-light text-dark border">{{ $d->tipe_jam_kerja ?? '' }}</span>
        </td>
        ...
        <td class="align-middle text-center">
            @if ($d->photo_in)
                <img class="img-circle zoomable-image"
                    src="{{ $d->photo_in != null ? asset('storage/absensi/' . $d->photo_in) : ' ' }}" alt="Foto Pulang"
        ...
        <td class="align-middle text-center">
            @if ($d->photo_out)
                <img class="img-circle zoomable-image"
                    src="{{ $d->photo_out != null ? asset('storage/absensi/' . $d->photo_out) : ' ' }}" alt="Foto Pulang"
                    style="width: 45px; height: 45px; cursor: pointer;">
            @else
                <i class="bx bx-camera-off text-muted font-20"></i>
            @endif
