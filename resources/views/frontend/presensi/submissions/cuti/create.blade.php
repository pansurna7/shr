                ...
                <div class="col">
                    ...
                    <div class="card border-0 shadow-sm bg-primary text-white mb-3">
                        <div class="card-body p-3">
                            ...
                            @if(date('n') <= 3 && $kuota_lalu_aktif > 0)
                                <div class="mt-2 p-2 bg-opacity-20 rounded" style="border: 1px dashed rgba(255,255,255,0.5)">
                                    <small style="font-size: 0.75rem; line-height: 1.2;">
                                        ⚠️ <strong>Perhatian!</strong><br>
                                        Sebanyak <strong>{{ $kuota_lalu_aktif }} hari</strong> adalah jatah tahun lalu yang akan <strong>hangus</strong> pada 31 Maret {{ date('Y') }}. Gunakan segera!
                                    </small>
                                </div>
                            @endif
                        </div>
                    ...
                    @if($boleh_cuti)
                        <div class="form-group mb-2">
                            ...
                            <input type="text" class="form-control datepicker" placeholder="Pilih Tanggal" name="tgl_izin" id="tgl_izin">
                        </div>
                            ...
                            <select name="jenis_cuti" id="jenis_cuti" class="form-control">
                                ...
                                @foreach ($leaves as $d)
                                    @php
                                        ...
                                        $isLakiLaki = (strtolower($employee->gender) == 'laki-laki' || strtolower($employee->gender) == 'l');
                                    @endphp
                    ...
                    @else
                        <div class="card border-0 shadow-sm mt-2">
                            <div class="card-body text-center py-5">
                                <ion-icon name="lock-closed-outline" class="text-warning" style="font-size: 4rem;"></ion-icon>
                                <h5 class="mt-3 fw-bold">Fitur Cuti Terkunci</h5>
                                <p class="text-muted small">
                                    Sesuai kebijakan perusahaan, pengajuan cuti hanya dapat dilakukan setelah minimal 1 tahun masa kerja.<br><br>
        ...
        flatpickr("#tgl_izin", {
            ...
            onChange: function(selectedDates, dateStr, instance) {
                ...
                if (selectedDates.length === 2) {
                    ...
                    while (cur <= end) {
                        let dayOfWeek = cur.getDay(); // 0: Minggu, 6: Sabtu
                    ...
                    let dateStr = selectedDates[0].toISOString().split('T')[0];
                    if (dayOfWeek !== 0 && dayOfWeek !== 6 && !holidays.includes(dateStr)) {
                        workDays = 1;
                    }
                }
                ...
                if (dateStr !== "") {
                    $.ajax({
                        ...
                        success: function(response) {
                            // Jika response > 0, berarti ada tanggal yang bentrok
                            if (response > 0) {
                                Swal.fire({
                                    title: 'Oops!',
                                    text: 'Anda sudah memiliki pengajuan pada rentang tanggal tersebut.',
                                    icon: 'warning',
                                }).then((result) => {
                                    // Reset form
                                    instance.clear();
