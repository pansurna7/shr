...
@section('content')
    ...
    <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
        <div class="breadcrumb-title pe-3">Postion List</div>
        <div class="ps-3">
    ...
    <div class="card">
        <div class="card-body">
            <div class="table-responsive table-bordered">
                <table class="table mb-0" id="tblPosition">
                    <thead class="table-light text-bold">
                        <tr>
                            <th scope="col" class="text-center align-middle">#</th>
                            <th scope="col" class="text-center align-middle">Departement</th>
                            {{-- <th scope="col" class="text-center align-middle">Kode Jabatan</th> --}}
                            <th scope="col" class="text-center align-middle">Nama Jabatan</th>
                        ...
                        @if (count($data)>0)
                            @foreach ($data as $item )
                                <tr>
                                    ...
                                    <td class="align-middle">{{$item->departement->name . "  " ."(" .$item->departement->code .")" }}</td>
                                    ...
                                    <td class="text-center align-middle">
                                        @can('position.edit')
                                            <a href="{{ route('position.edit', $item->id)}}" class="btn btn-sm btn-warning edit" id="{{$item->id}}">
                                                <i class="bx bx-pencil" data-toggle="tooltip" data-placement="top" title="Edit"></i>
                                                Edit
                                            </a>
                                        @endcan

                                        <form id="delete-form-{{ $item->id }}" action="{{ route('position.delete', $item->id) }}" method="POST" style="display: none;">
                                            @csrf
                                        </form>
                                        @can('position.delete')
    ...
    <div class="modal fade" id="modal-add-position" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content bg-light">
                ...
                <div class="modal-body text-white">
                    <form id="form-position" method="POST"  action="{{route('position.store')}}" class="needs-validation" novalidate>
                        @csrf
                        <div class="row mb-3">
                        ...
                        {{-- <div class="row mb-3">
                            <label for="groupName" class="col-sm-3 col-form-label">Kode Jabatan</label>
                            <div class="col-sm-9">
                                <div class="col-md-4">
    ...
    <div class="modal fade" id="modal-edit-position" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content bg-light">
                <div class="modal-header">
                    <h5 class="modal-title text-white">Update Position</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
        ...
        $(document).ready(function () {
            ...
            $(document).on('submit', '#form-edit-position', function(e) {
                ...
                $.ajax({
                    ...
                    dataType: 'json',
                    success: function (response) {
                        // $('#tblposition').DataTable().ajax.reload(null,false);
