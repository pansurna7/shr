    ...
    <div class="tab-content">
        <div id="step-1" class="tab-pane" role="tabpanel">
            ...
            <div class="input-group mb-3">
                <input type="text" class="form-control" name="nKtp" id="nKtp"
                    aria-label="Text input with checkbox" value="{{ old('nKTP', $employee->nomor_ktp ?? '') }}"
                    required />
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" name="fName" id="fName"
                    value="{{ old('fName', $employee->first_name ?? '') }}" required>
            </div>
            <div class="mb-3">
                <label for="lName" class="form-label">Last Name</label>
                <input type="text" class="form-control" name="lName" id="lName"
                    value="{{ old('lName', $employee->last_name ?? '') }}" required>
            </div>
            <div class="mb-3">
                <label for="tLahir" class="form-label">Tempat Lahir</label>
                <input type="text" class="form-control" name="tLahir" id="tLahir"
                    value="{{ old('tLahir', $employee->place_of_birth ?? '') }}" required>
            </div>

            <div class="mb-3">
                <label for="tglLahir" class="form-label">Tanggal Lahir</label>
                <div class="input-group date" id="datepicker">
                    <input type="text" class="form-control" id="tglLahir" name="tglLahir" placeholtglLahirDD"
                        autocomplete="off" value="{{ old('tglLahir', $employee->date_of_birth ?? '') }}" required>
            ...
            <div class="form-check form-check-inline">
                <input class="form-check-input" type="radio" name="gendre" value="laki-laki" id="gendre_laki"
            ...
            <div class="form-check form-check-inline">
                <input class="form-check-input" type="radio" name="gendre" value="perempuan" id="gendre_perempuan"
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="agama" name="agama" required>
                    ...
                    <option value="Kristen Protestan"
                        {{ ($employee->religion ?? '') == 'Kristen Protestan' ? 'selected' : '' }}>Kristen Protestan
                    </option>

                    {{-- Opsi 3: Kristen Katolik --}}
                    <option value="Kristen Katolik"
                        {{ ($employee->religion ?? '') == 'Kristen Katolik' ? 'selected' : '' }}>Kristen Katolik
                    </option>

                    {{-- Opsi 4-7: Lanjutkan pola yang sama untuk opsi lainnya --}}
                    <option value="Hindu" {{ ($employee->religion ?? '') == 'Hindu' ? 'selected' : '' }}>Hindu
                    </option>
                    <option value="Buddha" {{ ($employee->religion ?? '') == 'Buddha' ? 'selected' : '' }}>Buddha
                    </option>
                    <option value="Konghucu" {{ ($employee->religion ?? '') == 'Konghucu' ? 'selected' : '' }}>
                        Konghucu</option>
                    <option value="Lainnya" {{ ($employee->religion ?? '') == 'Lainnya' ? 'selected' : '' }}>Lainnya
                    </option>
                </select>
            </div>
            <div class="mb-3">
                ...
                <select class="form-select" id="statusNikah" name="statusNikah" required>
                    ...
                    <option value="Belum Kawin"
                        {{ ($employee->marital_status ?? '') == 'Belum Kawin' ? 'selected' : '' }}>Belum Kawin</option>
                    <option value="Kawin" {{ ($employee->marital_status ?? '') == 'Kawin' ? 'selected' : '' }}>Kawin
                    </option>
                    <option value="Cerai Hidup"
                        {{ ($employee->marital_status ?? '') == 'Cerai Hidup' ? 'selected' : '' }}>Cerai Hidup</option>
                    <option value="Cerai Mati"
                        {{ ($employee->marital_status ?? '') == 'Cerai Mati' ? 'selected' : '' }}>Cerai Mati</option>
                </select>
            </div>
            <div class="mb-3">
                <label for="jAnak" class="form-label">Jumlah Anak</label>
                <input type="text" class="form-control" name="jAnak" id="jAnak"
                    value="{{ old('jAnak', $employee->jumlah_anak ?? '') }}">
            </div>
            <div class="mb-3">
                ...
                <select class="form-select" id="pendidikan" name="pendidikan" required>
                    ...
                    <option value="SMA" {{ ($employee->education ?? '') == 'SMA' ? 'selected' : '' }}>SMA/SMK
                    </option>
                    ...
                    <option value="Tidak Sekolah"
                        {{ ($employee->education ?? '') == 'Tidak Sekolah' ? 'selected' : '' }}>Tidak Sekolah</option>
                </select>
            </div>
            <div class="mb-3">
                <label for="alamat" class="form-label">Alamat Lengkap</label>
                <textarea class="form-control" id="alamat" name="alamat" rows="3" required>{{ old('alamat', $employee->address ?? '') }}</textarea>
            </div>
            <div class="mb-3">
                <label for="handphone" class="form-label">Nomor Handphone</label>
                <input type="tel" class="form-control" id="handphone" name="handphone"
                    placeholder="Contoh: 081234567890" value="{{ old('handphone', $employee->mobile ?? '') }}"
            ...
            <div class="mb-3">
                ...
                <input type="file" name="avatar" id="avatar" class="dropify form-control mb-4"
                    data-allowed-file-extensions="jpg png jpeg" data-height="160" {{-- ✨ Perbaikan Logika Null Coalescing --}}
            ...
            <div class="row">
                <div class="col-6">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group date" id="datepicker">
                            <input type="text" class="form-control" id="tglKontrak" name="tglKontrak"
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group date" id="datepicker">
                            <input type="text" class="form-control" id="tglResign" name="tglResign"
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" name="gapok"
                            value="{{ old('gapok', $employee->gaji_pokok ?? '') }}" id="gapok" />
                    </div>

                    <div class="mb-3">
                        <label for="nRek" class="form-label">No. Rekening</label>
                        <input type="text" class="form-control" name="nRek" id="nRek"
                            value="{{ old('nRek', $employee->nomor_rekening ?? '') }}" />
                    </div>

                    <div class="mb-3">
                        <label for="pRek" class="form-label">Nama Pemilik Rekening</label>
                        <input type="text" class="form-control" name="pRek" id="pRek"
                            value="{{ old('pRek', $employee->rekening_atas_nama ?? '') }}" />
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Opsi Kehadiran</label>
                        <div class="form-check form-switch fs-6 mb-2">
                            <input type="checkbox" class="form-check-input" id="is_free_absent"
                                name="is_free_absent"
                                {{ old('is_free_absent', $employee->is_free_absent ?? false) ? 'checked' : '' }}>
                            <label class="form-check-label" for="is_free_absent">Free Absen (Bisa absen di mana
                                saja)</label>
                        </div>
                    </div>

                    <div id="location_select_container"
                        class="mb-3 {{ old('is_free_absent', $employee->is_free_absent ?? false) ? 'd-none' : '' }}">
                        <label for="locations" class="form-label">Lokasi Penempatan (Multiple)</label>
                        <select id="locations" name="location_ids[]" class="form-select select2" multiple="multiple"
                            data-placeholder="Pilih satu atau lebih lokasi...">
                            @foreach ($locations as $loc)
                                @php
                                    // Ambil ID lokasi yang sudah terasosiasi (untuk mode Edit)
                                    $selectedLocations = old(
                                        'location_ids',
                                        isset($employee) ? $employee->assigned_locations->pluck('id')->toArray() : [],
                                    );
                                @endphp
                                <option value="{{ $loc->id }}"
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-6">
                            ...
                            <div class="form-check form-switch fs-5">
                                <input type="checkbox" class="form-check-input" id="checkStatus" name="checkStatus"
        ...
        <div id="step-4" class="tab-pane" role="tabpanel">
            ...
            <div class="row">
                ...
                <div class="text-center mt-5">
                    ...
                    <a type="btn btn-light" href="{{ route('employee.index') }}" class="btn btn-light px-5"><i
                            class="lni lni-arrow-left-circle"></i>Cancel</a>
                </div>
            </div>
        </div>
    </div>
