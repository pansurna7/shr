                                ...
                                <tr>
                                    ...
                                    <td class="align-middle">
                                        <div class="d-flex flex-column">
                                            <span
                                                class="fw-bold text-primary">{{ $d->first_name . ' ' . $d->last_name }}</span>
                                            <small class="text-muted">{{ $d->position->name }}
                                                ({{ $d->position->departement->name }})</small>
                                        </div>
                                    </td>
                                            ...
                                            @if ($d->assigned_locations->count() > 0)
                                                @foreach ($d->assigned_locations as $loc)
                                                    <span class="badge bg-light text-dark border"><i
                                                            class="bx bx-map-pin"></i> {{ $loc->name }}</span>
                                                @endforeach
                                            @else
                                    ...
                                    <td class="text-center align-middle">
                                        ...
                                        <form id="delete-form-{{ $d->id }}"
                                            action="{{ route('employee.delete', $d->id) }}" method="POST"
                                            style="display: none;">
                                            @csrf
                                        </form>
                                        @can('employee.delete')
